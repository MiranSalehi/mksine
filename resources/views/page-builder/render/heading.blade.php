@php
    ...
    $levelClasses = match ($level) {
        ...
        'h6' => 'text-base md:text-lg font-semibold tracking-tight text-zinc-600 dark:text-zinc-400',
        default => 'text-2xl md:text-3xl font-bold tracking-tight',
    };
@endphp

<{{ $level }} class="{{ $levelClasses }} {{ $alignmentClasses }} text-balance text-zinc-900 dark:text-zinc-50 mb-6 md:mb-8">
    {{ $text }}
</{{ $level }}>
