...
- **`mksine:install` publishes Shield / Spatie Permission assets** (`filament-shield-config`, `permission-config`, `permission-migrations`) when Filament Shield is installed. Run `shield:generate --all` after install; the installer no longer registers Spatie’s generic `mksine:install` stub (only `MksineInstallCommand`).
