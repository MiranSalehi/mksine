...
and `root` pointing at **`public`**. The exact `location ~ \.php$` block must pass the correct `SCRIPT_FILENAME` to PHP-FPM.
...
Livewire registers either **`/livewire/livewire.js`** or **`/livewire/livewire.min.js`** depending on `config('app.debug')` (non-minified when debug is true). Blade output should match; mismatches can occur if:
...
After changing `.env` in production, run `php artisan config:clear` or rebuild config cache intentionally. See also the web server sections above — a 404 from nginx usually means routing, not Livewire itself.
...
- [50-troubleshooting.md](50-troubleshooting.md) — short symptom → link here
- [99-validation-checklist.md](99-validation-checklist.md) — production web checks
- [30-commands-reference.md](30-commands-reference.md) — `mks:release-archive`
