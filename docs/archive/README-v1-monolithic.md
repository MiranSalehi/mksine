...
- PHP 8.2 or higher (matches `composer.json`: `^8.2`)
- Laravel 11 or higher (use a version compatible with Filament 4)
- Filament 4
...
### User model, auth, and Filament (recommended)

MKSine centralizes the CMS user class in **`config/mksine.php`** as **`user_model`** (defaults to `App\Models\User` when you publish config). The Filament **User** resource resolves its Eloquent model from this value.

When **`sync_auth_user_model`** is `true` (default), the package aligns these at boot so you do not duplicate the same FQCN in `config/auth.php` and Filament Shield:

- `auth.providers.users.model`
- `filament-shield.auth_provider_model`

Toggle via config **`sync_auth_user_model`** or env **`MKS_CMS_SYNC_AUTH_USER_MODEL`**. Set to `false` only if you intentionally use a different auth model than `mksine.user_model` and will maintain both configs yourself.
...
`php artisan mks:discover` **always** scans the package tree `src/Core/Listeners` inside `miran/mksine` and syncs matching classes to `mks_hooks`.

To discover listeners under your application (or any other directory), add paths to **`config/mksine.php`** → **`hooks.discovery_paths`** (array of paths). Use absolute paths or paths relative to the app base directory (they are resolved with `base_path()`). Example:

```php
// config/mksine.php (published)
'hooks' => [
    'discovery_paths' => [
        app_path('Hooks/Listeners'),
    ],
    // ...
],
```
...
The command will:
1. Scan each configured directory recursively for PHP classes
2. Detect `MksineListenerInterface`, `FormHookListenerInterface`, and `TableHookListenerInterface` implementations
3. Sync rows in `mks_hooks` (it does **not** write `bootstrap/cache/mks_hook_discovery.php`; that cache file is unused by the command today)

At runtime, event listener **registration** from the database is handled when the package boots; priorities and enabled/disabled state are applied from `mks_hooks` via `HookStateRepository` when events are dispatched.
...
`Hooks::setPriority()` / `HookManager::setPriority()` are **deprecated and have no effect**. Use the database (or the listener’s `priority()` method for the initial sync) instead.
...
1. **Resolve paths**: Package `Core/Listeners` plus every existing directory in `config('mksine.hooks.discovery_paths')` (missing entries are skipped with a warning).
2. **Scan files**: Recursively load PHP files and detect hook listener interfaces.
3. **Merge results**: Later paths override earlier ones when the same form/table hook name or the same `listener_class` appears more than once.
4. **Sync with database**: Insert/update rows in `mks_hooks` via `DiscoveryService` (existing `is_enabled` flags are preserved where the implementation supports it).

The `mksine.hooks.cache_discovery` config flag and `bootstrap/cache/mks_hook_discovery.php` are **not** read or written by this command in the current release.

### Manual Discovery (programmatic)

```php
use Miran\Mksine\Core\Services\DiscoveryService;

$discovery = app(DiscoveryService::class);
$path = app_path('Hooks/Listeners');
$batch = $discovery->discoverAll($path);

$discovery->syncListeners($batch['listeners']);
$discovery->syncFormHooks($batch['form_hooks']);
$discovery->syncTableHooks($batch['table_hooks']);
```
...
1. **Clear State Cache**: Delete `bootstrap/cache/mks_hook_state.php` (if your deployment uses it)
2. **Re-run Discovery**: Run `php artisan mks:discover` (the command does not persist discovery output to `bootstrap/cache/mks_hook_discovery.php` today; that file is only relevant if you add custom tooling that writes it)
3. **Application cache**: Run `php artisan cache:clear` if you suspect stale config or cache drivers
...
This creates `{plugin_root}/{name}/` with (`{plugin_root}` = `base_path(config('mksine.plugins_path'))`):
...
3. Install npm deps and build: `cd {plugin_root}/{name} && npm install && npm run build`
...
If you add a Composer dependency **inside** a plugin (`cd {plugin_root}/{id} && composer require …`), many third-party packages document a **`php artisan …:install`** step or **`vendor:publish`** for config and migrations. Those commands assume the package is installed on the **host Laravel app** and usually write under the application’s `config/` and `database/migrations/`. A package that exists **only** under `{plugin_root}/{id}/vendor/` is not wired the same way at the app root, and **you should not rely on running those installers on production** when plugins are deployed as ZIPs without Composer.

**Use MKSine’s plugin-local workflow instead:**

1. **Define a preset** — Add `{plugin_root}/{id}/publishes/{preset}.json` describing where the package lives under `vendor/` and what to copy into the plugin (optional `config.from` / `config.to`, optional `migrations` list). The scaffolded plugin includes `publishes/README.md` with the JSON shape.
2. **Register a console command** in that plugin’s `boot()` that calls `Miran\Mksine\Core\Plugins\Publishing\PluginVendorPublishRunner` (pass the plugin manifest, preset name, console I/O, and `--force` if you need to overwrite config). Example: `php artisan {your-plugin-id}:publish-vendor {preset}` from the **app root** (`--force` overwrites published config targets).
...
- **Standard Resource**: fully customizable `UserResource`; the managed model is **`config('mksine.user_model')`**, not a hard-coded `App\Models\User`.
- **Dedicated Components**: Segregated `UserForm` and `UserTable` classes for better code organization.
- **Role & Permission Ready**: Works with Spatie Laravel Permission and Filament Shield; Shield’s **`auth_provider_model`** is kept in sync with **`mksine.user_model`** when **`sync_auth_user_model`** is enabled (see [User model, auth, and Filament](#user-model-auth-and-filament-recommended) under Installation).
- **Host app contract**: The concrete user class should implement **`FilamentUser`** (and **`HasPanelShield`** if Shield gates panel access) so administrators are not locked out with 403 after swapping the model.
